...
class ClaudeCodeManager:
    ...
    def setup_environment(self, env_name: Optional[str] = None) -> bool:
        ...
        if env_name is None:
            questions = [
                inquirer.Text("name", message="What is the name of this environment?", validate=lambda _, x: len(x) > 0)
            ]
        ...
        if self.config_manager.get_environment_config(env_name):
            ...
            questions = [inquirer.Confirm("overwrite", message="Overwrite?", default=False)]
            answers = inquirer.prompt(questions)
            if not answers or not answers["overwrite"]:
                return False

        # Get environment description
        questions = [inquirer.Text("description", message="Provide a description for this environment:")]
        answers = inquirer.prompt(questions)
        if not answers:
            return False
        ...
        while True:
            questions = [inquirer.Confirm("add_repo", message="Add a repository?", default=True)]
            answers = inquirer.prompt(questions)
            if not answers or not answers["add_repo"]:
                break

            questions = [
                inquirer.Text("url", message="Repository URL:", validate=lambda _, x: len(x) > 0),
                inquirer.Text("path", message="Local path (relative to work dir, leave empty for root):"),
        ...
        while True:
            questions = [inquirer.Confirm("add_command", message="Add a scaffold command?", default=True)]
            answers = inquirer.prompt(questions)
            if not answers or not answers["add_command"]:
                break

            questions = [inquirer.Text("command", message="Command:", validate=lambda _, x: len(x) > 0)]
            cmd_answers = inquirer.prompt(questions)
            if not cmd_answers:
                continue

            cmd_config = {"command": cmd_answers["command"]}
            env_config["scaffold_commands"].append(cmd_config)

        # Configure claude.md content
        print_info("Configure claude.md content:")
        questions = [
            inquirer.Confirm("edit_claude_md", message="Edit claude.md content in an external editor?", default=True)
        ]
        answers = inquirer.prompt(questions)
    ...
    def choose_environment(self, env_name: Optional[str] = None, instance_id: Optional[str] = None) -> bool:
        ...
        if instance_id is None:
            ...
            questions = [inquirer.List("instance_id", message="Select an instance:", choices=formatted_instances)]
            answers = inquirer.prompt(questions)
            if not answers:
                return False
        ...
        if action == "terminal":
            ...
            # Confirm deletion
            questions = [
                inquirer.Confirm("confirm", message="Are you sure you want to delete this instance?", default=False)
            ]
            answers = inquirer.prompt(questions)
    ...
    def delete_environment_instance(self, instance_id: Optional[str] = None, env_name: Optional[str] = None) -> bool:
        ...
        if instance_id is None:
            ...
            # Ask user to select an instance
            questions = [
                inquirer.List("instance_id", message="Select an instance to delete:", choices=formatted_instances)
            ]
            answers = inquirer.prompt(questions)
            if not answers:
                return False
    ...
    def list_env_types(self) -> bool:
        ...
        # Print table
        columns = [{"key": "name", "header": "Name", "style": "bold"}, {"key": "description", "header": "Description"}]
